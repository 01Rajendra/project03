...
/**
 * Smart Light List functionality controller. Used to perform Search, List and
 * Delete operations.
 *
 * @author Rajendra Singh
 *
 */
@WebServlet(name = "SmartLightListCtl", urlPatterns = { "/ctl/SmartLightListCtl" })
public class SmartLightListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(SmartLightListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("SmartLightListCtl doGet Start");
		...
		log.debug("SmartLightListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("SmartLightListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					SmartLightDTO deleteDto = new SmartLightDTO();

					for (String id : ids) {

						deleteDto.setLightId(DataUtility.getLong(id));
			...
			dto = (SmartLightDTO) populateDTO(request);
		...
		log.debug("SmartLightListCtl doPost End");
	}
